...
onMounted(() => {
    ...
    const limitedTitle = props.title.substring(0, 260);
    const limitedDescription = props.description.substring(0, 260);

    const vkText = `${limitedTitle}. ${limitedDescription}...Читайте далее на ${currentUrl.value}`;
    vkShareUrl.value = `https://vk.com/share.php?url=${encodeURIComponent(currentUrl.value)}&title=${encodeURIComponent(limitedTitle)}&description=${encodeURIComponent(limitedDescription)}&image=${encodeURIComponent(props.image)}&comment=${encodeURIComponent(vkText)}`;

    const whatsappText = `${limitedTitle}. ${limitedDescription}...Читайте далее на ${currentUrl.value}`;
    whatsappShareUrl.value = `https://wa.me/?text=${encodeURIComponent(whatsappText)}`;

    const telegramText = `${limitedTitle}. ${limitedDescription}`;
    const limitedTelegramText = telegramText.substring(0, 260);
    telegramShareUrl.value = `https://t.me/share/url?url=${encodeURIComponent(currentUrl.value)}&text=${encodeURIComponent(limitedTelegramText)}...Читайте далее на xcards.ru`;
});
