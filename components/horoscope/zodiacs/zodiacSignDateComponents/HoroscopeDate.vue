...
<template>
    <section class="horoscope_date">
        <div class="container">
            <div class="wrapper">
                <component :is="zodiacInfo[sign].image" class="img"></component>
                <h1 class="title">
                    {{ props.dateInfo.russianDate + props.dateInfo.genitiveCase }}
                </h1>

                <p class="desc">{{ props.text }}</p>
                <SocialLinks :title="props.dateInfo.russianDate + props.dateInfo.genitiveCase" :description="props.text"></SocialLinks>
            </div>
        </div>
    </section>
</template>
